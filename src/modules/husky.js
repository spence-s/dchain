...
async function manageHusky() {
  ...
  const husky = (command) => spawn(huskyExec, command, { stdio: 'ignore' });
  ...
  if (huskyUpgradePath) {
    ...
    if (huskyUpgradePath === '1') {
      let { config, filepath } = await cosmiconfig('husky').search(this.cwd);
